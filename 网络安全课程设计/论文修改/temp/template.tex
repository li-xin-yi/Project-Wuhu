...
The labeled data in this experiment are randomly selected from the data set. In order to observe the influence of the number of labeled data on classification results, We choose 10\%, 20\% and 90\% data  from the whole data set as the labeled successively. Then we design and run the experiments on Matlab R2010a. When updating the labeled data set following Rescale method, the cost of samples decrease by more than 50\%, those samples will be removed from the labeled data set, that is to say, we set $ts = 50\% $. The termination criterion for the iteration of CS-LGC Algorithm is that the difference of global costs between the lastest two iteration, which is the value of $threshold$, reaches 10\% or less. All the experimental data shown following are the average results of 50 repeated experiments.
